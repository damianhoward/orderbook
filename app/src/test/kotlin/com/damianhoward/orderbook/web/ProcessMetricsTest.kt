...
class ProcessMetricsTest {
    ...
    fun `peak and post-collection are summed across heap pools`() {
        ...
        assertTrue(metrics.contains("orderbook_jvm_heap_peak_bytes 150"), metrics)
        assertTrue(metrics.contains("orderbook_jvm_heap_post_gc_bytes 50"), metrics)
    }
    ...
    fun `non-heap pools are excluded from both`() {
        ...
        assertTrue(metrics.contains("orderbook_jvm_heap_peak_bytes 100"), metrics)
        assertTrue(metrics.contains("orderbook_jvm_heap_post_gc_bytes 30"), metrics)
    }
    ...
    fun `a heap not yet collected publishes no live set at all`() {
        ...
        assertFalse(metrics.contains("orderbook_jvm_heap_post_gc_bytes"), metrics)
    }
    ...
    fun `no heap pools publishes neither gauge`() {
        ...
        assertFalse(metrics.contains("orderbook_jvm_heap_peak_bytes"), metrics)
        assertFalse(metrics.contains("orderbook_jvm_heap_post_gc_bytes"), metrics)
    }
    ...
    fun `the real JVM answers both, and the peak is not below the live set`() {
        ...
        val peak = valueOf(metrics, "orderbook_jvm_heap_peak_bytes")
        val live = valueOf(metrics, "orderbook_jvm_heap_post_gc_bytes")
